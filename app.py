...
# --- 2. IL TUO LINK PIANO B (GOOGLE SCRIPT) ---
...
# --- 4. FUNZIONI DATI (VERSIONE SUPER-STABILE) ---
def carica_prenotazioni():
    try: 
        df = conn.read(worksheet="Prenotazioni", ttl=0)
        if df is not None and not df.empty:
            df.columns = [c.strip() for c in df.columns]
            # Forza la pulizia totale delle date e dei nomi
...
st.markdown("""
    ...
    th, td { padding: 4px; text-align: center; border: 1px solid #eee; min-width: 95px; height: 70px; vertical-align: middle; }
    ...
    .cell-booked { background: #ffcdd2 !important; color: #b71c1c !important; font-weight: bold; font-size: 11px; border-left: 5px solid #d32f2f !important; display: table-cell; vertical-align: middle; }
...
# --- 7. MAIN UI ---
def main():
    st.markdown(f"<h2 style='text-align:center; color:#2e7d32;'>VICTORY RADAR PRO {st.session_state.anno}</h2>", unsafe_allow_html=True)
    ...
    for ns, info in STRUTTURE.items():
        ...
        for d in range(1, num_days + 1):
            ...
            if k in confl: 
                ...
                nome_c = str(m.iloc[0]["Nome"])[:10].upper()
